...
impl<'a> MoveGen<'a> {
    ...
    /// iterate captures for the next MVV piece type, or remaining moves
    pub fn set_next_mvv(&mut self) {
        match self.mvv_mask_type {
            Some(Piece::Queen) => {
                self.filter_captures(Some(Piece::Rook));
            },
            Some(Piece::Rook) => {
                self.filter_captures(Some(Piece::Bishop));
            },
            Some(Piece::Bishop) => {
                self.filter_captures(Some(Piece::Knight));
            },
            Some(Piece::Knight) => {
                self.filter_captures(Some(Piece::Pawn));
            },
            Some(Piece::Pawn) => {
                self.filter_captures(None);
            },
            _ => {
                // this should never happen
                panic!("next_mvv_mask called when mvv_mask_type is None");
            }
        }
    }
